...
def data_load(args):
    ...
    dset_loaders["eval"] = DataLoader(dsets["eval"], batch_size=train_bs, shuffle=False, num_workers=0, drop_last=False)

    train_size = len(dsets["target"])
    print("train size:",train_size)

    dsets["test"] = ImageList_test(txt_test, transform=image_test())
    dset_loaders["test"] = DataLoader(dsets["test"], batch_size=train_bs*3, shuffle=False, num_workers=0, drop_last=False)
...
def cal_acc(loader, netF, netB, netC, memory, flag=False):
    ...
    if flag:
        ...
        acc = ' '.join(aa)
        return aacc, acc, auc, m_acc*100, m_num
    else:
        return accuracy*100, auc, mean_ent, m_acc*100, m_num
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.dset=='VISDA-C':
                acc_s_te, acc_list, auc_s_te, m_acc, m_num = cal_acc(dset_loaders['test'], netF, netB, netC, memory, True)
                log_str = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}%. AUC = {:.2f}%. \n The memory bank labeled numbers:{} with acc {:.2f}%'.format(args.name, iter_num, max_iter, acc_s_te, auc_s_te, m_num, m_acc) + '\n' + acc_list
                ...
                acc_s_te, auc_s_te, _ , m_acc, m_num = cal_acc(dset_loaders['test'], netF, netB, netC, memory, False)
                log_str = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}%. AUC = {:.2f}%. \n The memory bank labeled numbers:{} with acc {:.2f}%'.format(args.name, iter_num, max_iter, acc_s_te, auc_s_te, m_num, m_acc)
                if best_acc < acc_s_te:
                    best_acc = acc_s_te
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='office-home', choices=['VISDA-C', 'office', 'office-home', 'office-caltech', 'oct'])
