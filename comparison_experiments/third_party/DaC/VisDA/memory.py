...
class MemoryBank(nn.Module):
    ...
    def forward(self, inputs, inputs_s, inputs_s1, indexes, k=10):
        ...
        target_label = self.pred_labels[target_idx].clone().to(dev)
        source_label = self.pred_labels[source_idx].clone().to(dev)
        
        ad_sim_target = torch.zeros(self.args.class_num,B).float().to(dev)
        ad_sim_target.index_add_(0, target_label, inputs_out.t().contiguous()[target_idx])
        t_nums = torch.zeros(self.args.class_num, 1).float().to(dev)
        t_nums.index_add_(0, target_label, torch.ones(self.num_samples,1).to(dev)[target_idx].float())
        ad_sim_source = torch.zeros(self.args.class_num,B).float().to(dev)
        ad_sim_source.index_add_(0, source_label, inputs_out.t().contiguous()[source_idx])
        s_nums = torch.zeros(self.args.class_num, 1).float().to(dev)
        s_nums.index_add_(0, source_label, torch.ones(self.num_samples,1).to(dev)[source_idx].float())
