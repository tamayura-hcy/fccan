...
def valid(args, model, ad_net, writer, test_loader, global_step):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        if len(all_probs) == 0:
            ...
            all_probs[0] = np.append(
                all_probs[0], probs.detach().cpu().numpy(), axis=0
            )
    ...
    all_probs, all_label = all_probs[0], all_label[0]
    ...
    if args.dataset == 'visda17':
        return accuracy, auc, classWise_acc
    else:
        return accuracy, auc, None
...
def train(args, model):
    ...
    for global_step in range(1, t_total):
        ...
        if global_step % args.eval_every == 0 and args.local_rank in [-1, 0]:
            accuracy, auc, classWise_acc = valid(args, model, ad_net_local, writer, test_loader, global_step)
            ...
            logger.info("Current Best AUC: %2.5f" % best_auc)
